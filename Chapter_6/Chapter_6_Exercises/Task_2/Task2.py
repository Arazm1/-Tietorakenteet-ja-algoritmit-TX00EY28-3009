...
class HashTable():
    ...
    def put(self, key, value):
        ...
        Returns: None
        """
        key_len = self._hash(key)
        
        #Check if the key already exist
        existing_index = self._find_key(key_len, key)

        #If the key exist already, update the value
        if existing_index is not None:
            self.slots[existing_index].value = value
            return
        
        #Key doesnt exist already, find a free index for it
        free_index = self._find_free_slot(key_len)

        # Create a new HashItem and place it in the correct slot
