...
class Graph:
    ...
    def get_edges(self):
        ...
        for secondary_map in self._adj_map.values():
            # update() as it can append multiple values
    ...
    def get_edge(self, u, v):
        ...
        """
        # u: The source vertex (where the edge starts).
        # v: The destination vertex (where the edge ends).
...
"""
Start at vertex u
Everytime moves somewhere new, saves it to v
It it goes somewhere already visited, stops
...
def DFS(graph, u, visited=None):
    ...
    # Mark current node as visited (u is root so parent is None if not set)
    if u not in visited:
        visited[u] = None
...
def BFS(graph, start):
    ...
    currently_to_visit = [start]

    while currently_to_visit:
        next_to_visit = []
        #Iterate the vertices in current
        for u in currently_to_visit:
            for v in graph.get_adjacent_vertices(u):
                if v not in visited:
                    visited[v] = u
                    next_to_visit.append(v)

        currently_to_visit = next_to_visit
